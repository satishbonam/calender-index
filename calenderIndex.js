...
function requestApi(cCode, year, inputDate, display) {
    ...
    if (cCode == checkCountryCode && checkCountryCode !== null && data.response.holidays[0].date.iso.split('-')[0] == year) {
        display(inputDate)
    } else {
        alert("request sent")
        var url = "https://calendarific.com/api/v2/holidays?api_key=ded5fbb7b291be5ed5a2e120e742e3d9fc57b781&country=" + cCode + "&year=" + year
        xhr.open("GET", url)
        xhr.send()
        xhr.onload = function () {
            res = xhr.response
